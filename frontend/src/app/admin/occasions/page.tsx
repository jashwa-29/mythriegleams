        ...
        <div className="p-6 max-w-[1600px] mx-auto space-y-6 bg-white min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 pb-6">
                <div>
                    ...
                    <p className="text-xs text-zinc-500 font-medium">Manage occasions — festivals, parties and special events with categories and subcategories.</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={openAddModal} className="flex items-center gap-2 bg-zinc-900 text-white px-4 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider hover:bg-black transition-all">
                        <Plus size={14} />
            ...
            <div className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden">
                ...
                ) : mainOccasions.length === 0 ? (
                    <div className="py-20 flex flex-col items-center gap-3">
                        <Gift className="w-8 h-8 text-zinc-200" />
                        <p className="text-[10px] uppercase font-bold text-zinc-300 tracking-widest">No Occasions Yet</p>
                    </div>
                ) : (
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded border border-zinc-100 bg-zinc-50 overflow-hidden shrink-0 group-hover:scale-105 transition-transform duration-500">
                                                            {occ.image ? <img src={getImageUrl(occ.image)} className="w-full h-full object-cover" /> : <Gift className="text-zinc-200 m-auto" size={16} />}
                                                        </div>
                                                        <div>
                                                ...
                                                <tr key={sub._id} onClick={() => setInspectedOccasion(sub)} className="hover:bg-zinc-50/40 transition-all text-xs cursor-pointer group bg-zinc-50/30">
                                                    <td className="px-6 py-3">
                                                        <div className="flex items-center gap-4 pl-8">
                                                            ...
                                                            <div className="w-8 h-8 rounded border border-zinc-100 bg-white overflow-hidden shrink-0 group-hover:scale-105 transition-transform duration-500">
                                                                {sub.image ? <img src={getImageUrl(sub.image)} className="w-full h-full object-cover" /> : <Gift className="text-zinc-200 m-auto" size={12} />}
                                                            </div>
                    ...
                    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-zinc-200 overflow-hidden">
                            ...
                            <div className="p-8 space-y-8">
                                <div className="flex gap-6">
                                    <div className="w-24 h-24 rounded-xl border border-zinc-200 overflow-hidden bg-zinc-50 shadow-inner shrink-0">
                                        {inspectedOccasion.image ? <img src={getImageUrl(inspectedOccasion.image)} className="w-full h-full object-cover" /> : <Gift className="text-zinc-200 m-auto mt-7" size={24} />}
                                    </div>
                                    <div className="space-y-2 flex-1">
                                        <h4 className="text-lg font-bold text-zinc-900 tracking-tight break-words">{inspectedOccasion.name}</h4>
                            ...
                            <div className="p-6 bg-zinc-50 border-t border-zinc-100 flex items-center justify-between">
                                <button onClick={() => setDeleteModalOpen(true)} className="flex items-center gap-2 text-rose-500 hover:text-rose-700 font-bold text-[10px] uppercase tracking-widest transition-all"><Trash2 size={16}/> Delete</button>
                                <div className="flex gap-2">
                                    <button onClick={() => { openEditModal(inspectedOccasion); setInspectedOccasion(null); }} className="px-6 py-2 bg-zinc-200 text-zinc-900 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-zinc-300 transition-all"><Edit size={12} className="inline mr-1.5" />Edit</button>
                                    <button onClick={() => setInspectedOccasion(null)} className="px-6 py-2 bg-zinc-900 text-white rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-black transition-all">Close</button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                ...
                <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-white rounded-2xl p-8 overflow-hidden shadow-2xl border border-zinc-200">
                        <div className="flex items-center justify-between mb-8 pb-4 border-b border-zinc-100">
                            <div>
                                <h2 className="text-lg font-bold text-zinc-900 tracking-tight">{editingOccasion ? "Edit Occasion" : "New Occasion"}</h2>
                        ...
                        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5"><label className="text-[9px] font-bold uppercase tracking-widest text-zinc-400">Name</label><input {...register('name')} placeholder="Occasion Name" className="w-full bg-zinc-50 border border-zinc-200 rounded-lg p-3 text-xs font-bold focus:border-zinc-900 outline-none" /></div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <select {...register('parent')} className="w-full bg-zinc-50 border border-zinc-200 rounded-lg p-3 text-xs font-bold focus:border-zinc-900 outline-none appearance-none cursor-pointer">
                            ...
                            <div className="space-y-1.5"><label className="text-[9px] font-bold uppercase tracking-widest text-zinc-400">Image</label><div className="flex items-center gap-4"><label className="flex-1 border-2 border-dashed border-zinc-100 rounded-xl p-6 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-zinc-50"><ImageIcon size={20} className="text-zinc-300"/><span className="text-[9px] font-bold uppercase tracking-widest text-zinc-300">{editingOccasion?.image ? "Replace image" : "Upload image"}</span><input type="file" onChange={(e) => { const file = e.target.files?.[0]; if (file) { setImage(file); setPreview(URL.createObjectURL(file)); } }} className="hidden" accept="image/*" /></label>{preview && <div className="w-20 h-20 rounded-xl overflow-hidden border border-zinc-100"><img src={preview} className="w-full h-full object-cover" /></div>}</div></div>
